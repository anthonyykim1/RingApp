...
- Shortcuts shows a "Running your automation" notification banner when automation runs — **no native suppression** for "When I get a message" trigger. The "Notify When Run" toggle (iOS 15.4+) does NOT appear for message-based automations (Apple privacy policy). Shortcuts has NO entry in Settings → Notifications because the banner is a system activity indicator, not a `UNNotification`. Focus mode does NOT suppress it either — confirmed 2026-05-28 on iOS 26 (iPhone Air): Shortcuts isn't listable in Focus's "Silence Notifications From" (blocklist), and an allowlist + Time Sensitive OFF leaves the visual pop-up intact. No on-device workaround exists; the only escape is moving message detection off-device (Mac Studio). See `shortcuts.md` § "Suppressing the running-automation banner". Screen Time App Limits trick (0 min on Shortcuts) is unreliable and may block automations entirely.
...
3. **Shortcuts notification suppression** — ANSWERED 2026-05-10 / Focus FALSIFIED 2026-05-28 (iOS 26): no native suppression for message-trigger automations, and Focus mode does NOT work (neither blocklist nor allowlist). The banner is a system-level activity indicator with no on-device off-switch. See `shortcuts.md` § "Suppressing the running-automation banner". Re-check on major iOS releases in case Apple relaxes the block.
...
- **Shortcuts notification suppression:** Researched 2026-05-10 — no native suppression for message-trigger automations. `Notify When Run` toggle is hidden by Apple for Communication triggers (privacy-mandated). Shortcuts has no entry in Settings → Notifications because the banner is a system activity indicator, not a `UNNotification`. Focus mode does NOT suppress it either (confirmed 2026-05-28, iOS 26 / iPhone Air — neither blocklist nor allowlist). No on-device workaround; only escape is off-device (Mac) detection. Full write-up in `shortcuts.md` § "Suppressing the running-automation banner".
